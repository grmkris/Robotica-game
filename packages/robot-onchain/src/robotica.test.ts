...
describe("Robotica Onchain", () => {
	...
	describe("Game Entry", () => {
		...
		it("should generate valid enter game signature", async () => {
			...
			const signature = await signer.generateEnterGameSignature({
				user: addresses.player1,
				gameId,
			});
	...
	describe("Prize Claiming", () => {
		it("should generate valid claim prize signature", async () => {
			...
			const gameId = 1n;
			const signature = await signer.generateClaimPrizeSignature({
				amount,
				user: addresses.player1,
				gameId,
			});
	...
	describe("Error Cases", () => {
		...
		it("should fail when claiming with invalid amount", async () => {
			...
			const gameId = 1n;
			const signature = await player1.generateClaimPrizeSignature({
				amount,
				user: addresses.player1,
				gameId,
			});
