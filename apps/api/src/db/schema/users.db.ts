...
export const transactions = pgTable("transactions", {
	id: varchar("id", { length: 255 })
		.primaryKey()
		.$defaultFn(() => generateId("purrlonTxn")),
	userId: varchar("user_id", { length: 255 })
		.notNull()
		.references(() => users.id)
		.$type<UserId>(),
...
export const wallets = pgTable(
	"wallets",
	{
		...
		userId: varchar("user_id", { length: 255 })
			...
			.references(() => users.id)
			.$type<UserId>(),
		address: varchar("address", { length: 255 }).notNull().$type<EvmAddress>(),
		type: walletTypeEnum("type").notNull().$type<WalletType>(),
		chainId: numeric("chain_id").notNull().$type<WalletChainId>(),
		createdAt: timestamp("created_at", { withTimezone: true, mode: "date" })
			.defaultNow()
			.notNull(),
		updatedAt: timestamp("updated_at", { withTimezone: true, mode: "date" })
	...
	(table) => [
		...
		uniqueIndex("wallet_address_chain_idx").on(table.address, table.chainId),
	],
);
