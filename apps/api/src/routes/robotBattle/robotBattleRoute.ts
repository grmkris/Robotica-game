...
export const createBattleRoute = new OpenAPIHono<{
  ...
  async (c) => {
    ...
    try {
      ...
      // Fetch the last gameId and increment it
      const [{ maxGameId }] = await db
        .select({ maxGameId: sql<number>`max(game_id)` })
        .from(BattleTable);

      const newGameId = (maxGameId ?? 0) + 1; // Increment the last gameId
      ...
      const [battle] = await db
        ...
        .values({
          ...
          createdBy: user.id,
          gameId: newGameId, // Set the new gameId
        })
        .returning();
